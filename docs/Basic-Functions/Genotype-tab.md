...
Once a VCF dataset has been loaded into the view, select the region the region of interest by brushing the axis. Zoom in as needed to locate the region of interest. Once the axis is brushed, the selected SNPs will be displayed in the Genotype tab.
...
Once a region has been brushed and SNPs are displayed in the Genotype tab, genotypes for accessions can be added via a pop out menu from the Genotype tab. If working with genotype data generated by the AGG Strategic Partnership, you will need to search by accession number/genotype ID. To find the AGG accession number/genotype ID for an accession of interest, use <a href="https://genolink.plantinformatics.io/">Genolink</a> to <a href="https://docs.plantinformatics.io/Genolink/user-guide/#wild-text-search">search by accession name</a>.
